...
const ProductionPage = async () => {
    ...
    const config = await loadPageConfig('production');
    const portfolioItems = await loadPagePortfolio('production');
    const teamMembers = await loadPageTeam('production');
    const faqItems = await loadFAQ('production');
        ...
        <>
            <main>
                <HeroSection backgroundMedia={config.hero.backgroundMedia} mediaType={config.hero.mediaType}>
                    {/* Production Heading with StarFrame */}
